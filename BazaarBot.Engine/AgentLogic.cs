...
public class AgentLogic
{
    ...
    {
		if(!currentNode.IsLeaf){								//If not a leaf node
			if (Evaluate(currentNode, agent)) {				//Test the conditions
				if(currentNode.NodeTrue != null){
					Perform(currentNode.NodeTrue, agent);	//Branch to "if_true" node
				}
			}else {
				if(currentNode.NodeFalse != null){
					Perform(currentNode.NodeFalse, agent);	//Branch to "if_false" node
				}
			}
		}else{												//Do the actions
			foreach (var act in currentNode.Actions) {				
		...
		foreach (var c in currentNode.Conditions) {
			switch(c.Condition) {
				case "has":	//Do you have something?
					var has = false;
					foreach (var str in currentNode.Parameters.Values<string>()) {		//look at all the things
